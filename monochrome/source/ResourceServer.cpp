...
int ResourceServer::LoadSoundMem(const TCHAR* FileName)
{
	...
	// キーが無かった
	int sd = ::LoadSoundMem(FileName);
	// キーとデータをmapに登録
	_mapSound[FileName] = sd;

	return sd;
}

int	ResourceServer::LoadDivGraph(const TCHAR* FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int* HandleBuf)
{
	// キーの検索
